...
sid.tc.long.sum.bin$variable = substr(sid.tc.long.sum.bin$variable, start = 3, stop = str_length(sid.tc.long.sum.bin$variable))

sid.tc.long.sum$vale = as.factor(sid.tc.long.sum$vale)
sid.tc.long.sum$variable = as.factor(sid.tc.long.sum$variable)
sid.tc.long.sum$variable = substr(sid.tc.long.sum$variable, start = 3, stop = str_length(sid.tc.long.sum$variable))
sid.tc.long.sum$hit = as.factor(sid.tc.long.sum$hit)
sid.tc.long.sum$hit = fct_recode(sid.tc.long.sum$hit,
                               "miss" = "0", "hit" = "1")
...
sid.tc.long.sum.bin.inv$variable = substr(sid.tc.long.sum.bin.inv$variable, start = 3, stop = str_length(sid.tc.long.sum.bin.inv$variable))
...
sid.tc.long.sum.inv$variable = substr(sid.tc.long.sum.inv$variable, start = 3, stop = str_length(sid.tc.long.sum.inv$variable))
sid.tc.long.sum.inv$hit = as.factor(sid.tc.long.sum.inv$hit)
sid.tc.long.sum.inv$hit = fct_recode(sid.tc.long.sum.inv$hit,
                               "miss" = "0", "hit" = "1")
...
plotBin = function(d, area, val){
  ...
  d1$trialtype_f = factor(d1$trialtype_f)
  tcplot = ggplot(
    d1, aes(x = TR, y = mean, group = trialtype_f, shape = trialtype_f)) + 
    geom_errorbar(aes(ymin = mean-se.mean, ymax = mean+se.mean), width = 0.25) +
    geom_line(aes(linetype = trialtype_f, colour = trialtype_f), size = 1) +
    geom_point(aes(shape = trialtype_f, colour = trialtype_f, fill = trialtype_f), size = 3.5, shape = 21) +
...
plotHVM = function(d, area, val){
  d1 <- d %>%
    filter(variable==area & vale==val)
  tcplot = 
    ggplot(d1, aes(x = TR, y = mean, shape = hit, color = trialtype_f)) +
    geom_errorbar(aes(ymin = mean-se.mean, ymax = mean+se.mean), width = 0.25) +
    geom_point(aes(group = interaction(trialtype_f, hit)), size = 3.5) +
    geom_line(aes(color = trialtype_f, linetype = hit), size = 1) +
